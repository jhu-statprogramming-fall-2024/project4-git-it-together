...
#education <- p1_education %>%
#  group_by(state, year) %>%
#  reframe(
#    annual_change = mean(oty_month3_emplvl_pct_chg, na.rm = TRUE)
#  )

#education$year <- as.numeric(p1_education$year)
...
```{r}
# 
information_results <- process_and_model(dataset = information,
  directly target_variable = "oty_month3_emplvl_pct_chg",
  predictors = c("year_quarter_cont", "state")
)
...
```{r}
# 
leisure_results <- process_and_model(dataset = leisure,
  directly target_variable = "oty_month3_emplvl_pct_chg",
  predictors = c("year_quarter_cont", "state")
)
...
```{r}
professional_results <- process_and_model(dataset = professional,
  directly target_variable = "oty_month3_emplvl_pct_chg",
  predictors = c("year_quarter_cont", "state")
)
