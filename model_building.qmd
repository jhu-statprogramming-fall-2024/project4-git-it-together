...
#Machine learning model: Linear regression model to predict mean quarterly income 
...
professional_results <- process_and_model(
  dataset_path = "professional.xlsx",
  sheet_name = NULL,  # Specify sheet name if applicable
  target_variable = "oty_month3_emplvl_pct_chg",
  predictors = c("year_quarter_cont", "state")
)
...
```
```{r}
# Function to analyze and visualize the distribution of predicted_value (predicted % change in employment) to determine width of bins 
analyze_distribution <- function(data, column_name = "predicted_value") {
  library(ggplot2)
  ...
  # Suggest potential bins based on quantiles
  quantiles <- quantile(values, probs = seq(0, 1, 0.1))  # Deciles
  print("Quantiles (Deciles):")
  print(quantiles)
  
  return(list(summary_stats = summary_stats, quantiles = quantiles))
}

# Use the function on future_data_combined
distribution_info <- analyze_distribution(future_data_combined, "predicted_value")
```
